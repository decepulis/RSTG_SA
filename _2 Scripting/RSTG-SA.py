...
#####################################################################################
class dbManager():
    # input:  login = {'user':username, 'password':pwd, 'host':host, ['database': db, ...]}
    def __init__(self, login):
        ...
        try:
            ...
            if exc.errno == 2003:
                print('''Connection with database timed out. 
                      Attempting to run with cached data.''')
            else:
    ...
    def loadUsersFromDB(self):
        ...
        for(id,org,jira_key,name,display,email_address,url,time_zone,locale) in cursor:
              ...
              usersByID[id]            = {'key':jira_key, 'display':display}

        cursor.close()

        #usersByKey['nmcl']
        #usersByKey['stephen.fikes']
        #usersByKey['clebert']
        #usersByKey['jhalli']

        #usersByKey['anbaker']
        usersByKey['roger.martinez'] = 17
        usersByKey['dzonca'] = 61
        #usersByKey['cwatkins']
        #usersByKey['ganandan-Redhat']
        usersByKey['mario.fusco'] = 34
        usersByKey['andrew.collins'] = 355
        usersByKey['psiroky-redhat.com'] = 27


        usersByDisplay['Geoffrey De Smet'] = 48
        usersByDisplay['Youcef Hilem'] = 299
        usersByDisplay['Davide Scottara'] = 291
        usersByDisplay['Liz Claytton'] = 7
        usersByDisplay['Roger Martinez'] = 17
    ...
    def addProjectIssuesToGraph(self,project,graph):
        ...
        for(id,jira_id,jira_key,url,project,issue_type,priority,assignee,description,
            summary,timestamp,creator,watches,votes,has_questions,has_needs) in cursor:
            issue = "Issue "+ str(id)
            ...
            '''
            keywords = (word[0] for word in self.rake.run('{} {}'
                       .format(summary,description)) if word[1]>=1.0)
            ...
            if (assignee != None):
                  ...
                  try:
                      graph.addNode(assignee_label, 
                        type="person", 
                        display=self.getUser(id=assignee,ret='display')
                      )
            ...
            if (creator  != None): 
                  ...
                  try:
                      graph.addNode(creator_label, 
                        type="person", 
                        display=self.getUser(id=creator,ret='display')
                      )
    ...
    def addProjectCommentsToGraph(self,project,graph):
        print('Populating graph with comments and questions from project {}...'
              .format(project))

        query = '''SELECT * FROM jira_issue_comment WHERE issue IN 
            ( SELECT id FROM jira_issue WHERE project = {})'''.format(project)
        cursor = self.cnx.cursor()
        cursor.execute(query)

        for(id,jira_id,author,url,body,issue,created,updated,
            parsetree,is_question,is_needs) in cursor:
            ...
            if author:
                  ...
                  graph.addNode(author_label,
                    type="person", 
                    display=self.getUser(id=author, ret='display')
                  )
            ...
            for ref in refs:
                try:
                    ...
                    graph.addNode(user,
                        type="person", 
                        display=self.getUser(id=userid, ret='display')
                    )
    ...
    def findUserComment(self,userDisplay,comment):
        ...
        query = '''SELECT * FROM (SELECT * FROM jira_issue_comment WHERE author={}) 
                   AS q1 WHERE q1.body LIKE "{}%";'''.format(userid,comment)
        cursor = self.cnx.cursor()
        cursor.execute(query)

        # now we find all 
        comments = []
        for(id,jira_id,author,url,body,issue,created,
            updated,parsetree,is_question,is_needs) in cursor:
            comments.append({
                "comment": "Comment " + str(id),
                "issue":   "Issue "   + str(issue) 
                })
...
class graph():
    ...
    def __init__(self, **kwargs):
        self.weights = {'assignee':2, 'creator':2,'asked':4,'posted':3,
                        'referenced in':2,'question':1,'comment':1, 'keyword':4}
    ...
    #                      e.g. datetime.strptime(G.node[question]['timestamp'],
    #                                             self.timeformat)
            ...
            for edge in bft:
                ...
                for preNode in self.G.neighbors(postNode):
                    if 'activation' in self.G.node[preNode]:
                        ...
                        calcActivation = (self.G.node[preNode]['activation'] 
                                          * (1 - (weight * spaceDecay)))
    ...
    def numNodesAboveActivation(self,activationCutoff):
        return (sum(1 for n in self.G.nodes(data='activation') 
                    if (n[1] != None and n[1] >= activationCutoff)))
...
if __name__ == '__main__':
    ...
    login = {'user':'root', 'password':'foraging', 
             'host':'127.0.0.1', 'database':'network_t'}
    db = dbManager(login)
    ...
    data = {
        'activations'           : [],
        'distances'             : [],
        'nodesAboveActivation'  : defaultdict(list),
        'answerAboveActivation' : defaultdict(list),
        'nodesInRadii'          : defaultdict(list),
        'answerInRadius'        : defaultdict(list)
    }
    # == For Each Project ===========================================================
    projects = {"54 RQ2 DASHBUILDER.csv":1, "67 Drools answer set.csv":2, 
                "145 RQ2 IMMUNANT Answer set.csv":4, "243 JBTM answer set.csv":3}
    for f in glob.glob(os.getcwd()+'/answers/*.csv'):
        ...
        with open(answerCSV,encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                # -- Filter by Question --------------------------------------------
                try:
                    question = questionDB[body]["comment"]
                    issue    = questionDB[body]["issue"]
                except KeyError:
                    questions = db.findUserComment(asker,body)
                    question = None
                    issue    = None
                    # for all the user's comments that have the same body we pick 
                    for qu in questions: 
                        # the first one in our graph 
                        if g.nodeInGraph(qu["comment"]) and g.nodeInGraph(qu["issue"]):
                            question = qu["comment"]
                            issue    = qu["issue"]
                    ...
                    questionDB[body] = {"comment": question, "issue": issue}

                try:
                    q = loadObject(question)
                except FileNotFoundError: # we filter our graph by time
                    q = graph(graph = g.newSubgraphBeforeNode(question))
                ...
                # -- Spread and Analyze---------------------------------------------------------
                # q.spreadActivationSimple(question,issue,0.25) # Weightless
                q.spreadActivationSimple(question,None,0.1)
                q.saveAsGML(question)
                try: 
                    paths = list(q.findPaths(question, answer))
                    answerActivation = q.getNodeActivation(answer)
                    distance = len(paths[0])
                    print("Activation: {0:.2f}, Paths: {1:2d}, Distance: {2}, Sample: {3} "
                          .format(answerActivation, len(paths), distance, paths[0]))
                    data['activations'].append(answerActivation)
                    data['distances'].append(distance)

                    for i in range(1,100):
                        i_activation = 0.01*i
                        data['nodesAboveActivation'][i].append(
                            q.numNodesAboveActivation(i_activation))
                        data['answerAboveActivation'][i].append(
                            int(answerActivation >= i_activation))

                    for i in range(1,6):
                        data['nodesInRadii'][i].append(q.numNodesInRadius(i, question))
                        data['answerInRadius'][i].append(int(distance <= i))
    ...
    for k in data['nodesInRadii']: 
        prcAnswers = ( 
            100*sum(data['answerInRadius'][k])/len(data['answerInRadius'][k]))
        boxGraph   = percentile(data['nodesInRadii'][k],[0,25,50,75,100])

    plot = defaultdict(list)
    for k in data['nodesAboveActivation']: 
        prcAnswers = (
            100*sum(data['answerAboveActivation'][k])/len(data['answerAboveActivation'][k]))
        boxGraph   = percentile(data['nodesAboveActivation'][k],[0,25,50,75,100])

        plot[0].append(boxGraph[0])
        plot[25].append(boxGraph[1])
        plot[50].append(boxGraph[2])
        plot[75].append(boxGraph[3])
        plot[100].append(boxGraph[4])
        plot['prc'].append(prcAnswers)
